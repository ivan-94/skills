...
outputs(
    ...
    optional=[
        ...
        output(
            "mechanical_validation_evidence",
            type=Text,
            description="内置验证脚本的命令、结果和失败项；只证明 hard mechanical errors 状态。",
        ),
        output(
            "semantic_review_evidence",
            type=Text,
            description="按 review_dimensions 和用户约束逐项完成的语义审查证据。",
        ),
    ],
)
...
resources(
    ...
    references=[
        ...
        reference(
            ...
            purpose="frontmatter description 与 activate_when/do_not_activate_when 对齐示例。",
            when="需要生成或审查正文加载前的 frontmatter description",
        ),
        reference(
            "examples/linear-skill.md",
            purpose="普通线性 DSL-backed Skill 示例；适合无 CLI、无图工作流、无特殊审批生命周期的 Skill。",
            when="Skill 主要是一个普通线性 workflow，需要中性模板而不是 CLI、图或审批示例",
        ),
        reference(
            ...
            when="Skill 主要包装一个脚本或 CLI，且没有复杂审查维度、图工作流或多阶段工作流",
        ),
        reference(
            "examples/decisions-modes-safety.md",
            purpose="decision_rules、modes、safety_policy 和审批示例。",
            when="Skill 有模式、分支、fallback、安全边界或用户审批",
        ),
        reference(
            "examples/graph-workflow.md",
            purpose="workflow_graph、node、edge、join、termination 和 MCP 调用示例。",
            when="线性 workflow 无法表达 DAG、并行、join 或显式终止",
        ),
        reference(
            "examples/iteration-aggregation.md",
            purpose="loop、map_each、failure_policy 和聚合示例。",
            when="Skill 需要迭代、批量评估、多 case 检查或聚合结果",
        ),
        reference(
            "examples/subagent-delegation.md",
            purpose="call_subagent、隔离上下文、result_path 和条件必填输入示例。",
            when="Skill 需要把有边界的工作委托给子 Agent 并保留可审计交付路径",
        ),
        reference(
            "examples/human-review-vocabulary.md",
            purpose="call_skill、review_dimensions、validation 和人工审查闸门示例。",
            when="Skill 需要人工审查、委托其他 Skill 或定义审查维度",
        ),
    ],
)
...
workflow(
    [
        ...
        step("run_semantic_review", "按 quality_bar、validation checks、review_dimensions 和用户约束逐项做语义审查；记录每个维度的通过、问题或不适用原因。", reads=["contract_draft"], writes=["review_notes", "semantic_review_evidence"]),
...
modes(
    [
        ...
        mode(
            ...
            workflow=[
                step("load_review_scope", "读取目标 Skill、contract.pyi、contract 声明的 scripts/references/assets 公开接口，以及被引用的 examples；不要批量加载无关资源。", writes=["review_scope"]),
                ...
                step("check_calls_and_gates", "检查 call_* 是否只嵌入自然语言字符串；检查 ask_user、ask_when_uncertain、approval_required 等结构化用户闸门是否被保留。", reads=["review_scope"], writes=["findings"]),
...
decision_rules([
    ...
    when("Skill 需要教学型示例、审查维度、多 mode、复杂图工作流或跨 Skill 委托", then="保留必要高级结构，但仍在语义审查后执行奥卡姆剃刀"),
    ...
    when("任务需要持续人工审查、批准或生命周期边界", then="读取 examples/human-review-vocabulary.md 并使用 modes、workflow、call_human 和 safety_policy"),
...
safety_policy(
    ...
    must_not=[
        "不要把 call_human 当成 ask_user 或 approval_required 的替代品",
...
quality_bar(
    must=[
        ...
        "resources 声明每个 script、reference 和 asset 的用途、加载时机或公开接口",
        "写入前必须经过 run_semantic_review 和 apply_occams_razor，避免简单 Skill 膨胀",
        "语义审查必须逐项覆盖 review_dimensions 中定义的品味审查规则，并写入 semantic_review_evidence",
...
review_dimensions([
    ...
    "简洁密度：简单脚本型 Skill 不得套用复杂模板；同一规则不得在 modes、decision_rules、quality_bar、validation 或 examples 中重复展开。",
...
validation(
    [
        ...
        check("semantic_interface_and_behavior_closed", "语义审查：required inputs/outputs 被 workflow 或 modes 消费/产出，所有分支、循环和图工作流都有输出、停止条件或用户问题。"),
        check("semantic_user_gates_preserved", "语义审查：结构化用户输入闸门不被 call_human 替代，用户明确要求的约束必须保留在 contract 中。"),
        check("semantic_taste_review", "语义审查：review_dimensions 中每个品味维度都被检查，finding 必须说明违反的维度、证据和 Agent 影响；通过项和不适用项写入 semantic_review_evidence。"),
    ],
    on_failure="report",
)
